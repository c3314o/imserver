...
public class MessageContent extends BaseAction {
     ...
     * 
     * @param toId 消息接收者, 发送的目的地，messageType = 1时,这个id是userId， messageType = 2时，这个id是groupId
     * 
    ...
    private DataBuffer packageMessage(DataBuffer dataBuffer, Message message) {
        ...
        if (user != null) {
            dataBuffer.writeString(user.getUserInfo().getUname());
            dataBuffer.writeString(user.getUserInfo().getUnick());
            dataBuffer.writeString(user.getUserInfo().getAvatar());
        } else {
